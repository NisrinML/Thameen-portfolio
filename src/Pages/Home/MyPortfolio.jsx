...
export default function MyPortfolio() {
  ...
  return (
    <section className="portfolio--section" id="MyPortfolio" ref={ref}>
      <div className="portfolio--container-box">
        <div className="portfolio--container">
          <p className="sub--title">Recent Projects</p>
          <h2 className="section--heading skills--section--heading">My Portfolio</h2>
        </div>
        <div>
          <button className="btn btn-github" onClick={()=>handleMove('https://www.linkedin.com/in/thameen-qoronfli-926b68229/')}>
            <svg
        ...
        {data?.portfolio?.map((item, index) => (
          ...
          className="portfolio--section--card">
            ...
            <div className="portfolio--section--card--content">
              <div>
                <h2 className="portfolio--section--title" style={{textAlign:'center'}}>{item.title}</h2>
                <p className="text-md" style={{textAlign:'center'}}>{item.description}</p>
              </div>
              <p className={`text-sm portfolio--link ${item.path=='private'?'portfolio--link--private':''}`} onClick={()=>handleMove(item.path)}>
                {item.link}
                {item.path!="private"&&
                <svg
                  xmlns="http://www.w3.org/2000/svg"
